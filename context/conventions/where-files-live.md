...
- **All source lives under `src/`** - every language shares one `src/` (Python `src/<pkg>/`, MATLAB `src/+<pkg>/`); never a top-level `python/`, `matlab/`, or other language-named source folder, even alongside existing code.
