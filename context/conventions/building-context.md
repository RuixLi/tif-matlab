...
- **Current work** - a pointer to `context/jobs.md`, the current-work surface (`tracking-progress`). The work state itself lives on the board, never inline in `AGENTS.md` - so the entry point stays clean whether one effort runs or many.
